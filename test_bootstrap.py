...
def test_bootstrap_ci():
    ...
    with pytest(TypeError, match = 'bootstrap_stats must be numpy ndarray'):
...
def test_R_squared():
    ...
    X=np.ones((2,3))
    y=np.ones(2)
    with pytest(ValueError, match = 'must match length of y'):
        R_squared(X,y)
    with pytest(TypeError, match = 'must be numpy ndarray'):
        R_squared(1,y)
